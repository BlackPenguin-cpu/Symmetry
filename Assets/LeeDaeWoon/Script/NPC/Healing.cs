...
public class Healing : MonoBehaviour
{
    ...
    [SerializeField] Text healingGodlText;
    ...
    void ScreentoWorld()
    {
        ...
        healingRectPos.localPosition = Camera.main.WorldToScreenPoint(transform.localPosition + new Vector3(2.6f, -5f, 0));
    }
    ...
    void Healing_Price()
    {
        ...
        healingGodlText.text = healingGold.ToString();
    }

    IEnumerator Healing_Purchase()
    {
        if (Input.GetKeyDown(KeyCode.F) && isColiderCheck == false && GameManager.Instance._coin >= healingGold && isPurchaseCheck == true)
        {
            SoundManager.instance.PlaySoundClip("SFX_God_healling", SoundType.SFX);
            GameManager.Instance._coin -= healingGold;
    ...
    IEnumerator CloseHealingWindow()
    {
        timer = 0;
        SoundManager.instance.PlaySoundClip("SFX_Window", SoundType.SFX);
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if ((collision.CompareTag("Player") || collision.GetComponent<ITypePlayer>() != null) && isColiderCheck == true && isPurchaseCheck == true)
        {
            ...
            StartCoroutine(OpenHealingWindow());
        }
    }
